        ...
        try {
            ...
            if (status === "granted") {
                const { data } = await ContactsModule.getContactsAsync({
                    fields: [ContactsModule.Fields.PhoneNumbers, ContactsModule.Fields.Emails],
...
export const RecentsProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const fetchInitial = useCallback(async (showLoading = true) => {
        try {
            ...
            const result = await CallLogsModule.getCallLogs(PAGE_SIZE, 0);
            if (result && result.logs) {
                const logs = result.logs as CallLogProps[];
                setRawLogs(logs);
                setSections(groupCallsByDate(logs));
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            const { CallLogsModule } = require("../modules/dialer-module");
            const result = await CallLogsModule.getCallLogs(PAGE_SIZE, offsetRef.current);
            if (result && result.logs) {
                const newLogs = result.logs as CallLogProps[];
                if (newLogs.length > 0) {
                    setRawLogs((prev) => {
                        const combined = [...prev, ...newLogs];
                        setSections(groupCallsByDate(combined));
                        return combined;
                    });
                    setHasMore(result.hasMore as boolean);
                    offsetRef.current += PAGE_SIZE;
    ...
    return (
        <RecentsContext.Provider value={{ sections, loading, loadMore, hasMore, refresh }}>
            {children}
...
export const CallStateProvider = ({ children }: { children: React.ReactNode }) => {
        ...
        const checkCall = async () => {
            try {
                ...
                if (activeCall && activeCall.state !== 7) {
                    setCallState((prev) => {
                        if (prev?.state === activeCall.state && prev?.number === activeCall.number && prev?.name === activeCall.name) return prev;
                        return activeCall;
                    });
                } else {
