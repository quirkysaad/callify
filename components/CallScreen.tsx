...
const CallScreen = () => {
    ...
    const handleAnswer = useCallback(() => {
        try { CallLogsModule.answerCall?.(); } catch (_e) { }
    }, []);
    const handleReject = useCallback(() => {
        try { CallLogsModule.rejectCall?.(); } catch (_e) { }
    }, []);
    const handleEndCall = useCallback(() => {
        try { CallLogsModule.disconnectCall?.(); } catch (_e) { }
    }, []);

    const handleToggleMute = useCallback(() => {
        const newMuted = !isMuted;
        setIsMuted(newMuted);
        try { CallLogsModule.toggleMute?.(newMuted); } catch (_e) { }
    }, [isMuted]);

    const handleToggleSpeaker = useCallback(() => {
        const newSpeaker = !isSpeaker;
        setIsSpeaker(newSpeaker);
        try { CallLogsModule.toggleSpeaker?.(newSpeaker); } catch (_e) { }
    }, [isSpeaker]);

    const handleSendDtmf = useCallback((digit: string) => {
        setDtmfDigits((prev) => prev + digit);
        try { CallLogsModule.sendDtmf?.(digit); } catch (_e) { }
    }, []);
    ...
    const matchedContact = React.useMemo(() => {
        ...
        const matchTarget = cleanNumber.length > 7 ? cleanNumber.slice(-10) : cleanNumber;
    ...
    const isDialing = callState.state === 3 || callState.state === 9 || callState.state === 1;
    ...
    const displayName = callState.name || matchedContact?.name || callState.number;

    return (
        <Animated.View entering={SlideInDown.duration(400)} style={StyleSheet.absoluteFillObject} className="z-[9999] bg-background elevation-[999]">
            <SafeAreaView className="flex-1 justify-between py-5">
                <Animated.View entering={FadeInDown.delay(200).duration(400)} className="items-center mt-10">
                    <Text className="text-textSecondary text-base font-medium tracking-[1px] mb-2">{statusText}</Text>
                    <Text className="text-textPrimary text-[34px] font-light tracking-[-0.5px] mb-[6px]">{displayName}</Text>
                    {showKeypad && dtmfDigits ? (
                        <Text className="text-textPrimary text-xl font-normal tracking-[2px]">{dtmfDigits}</Text>
                    ) : (
                        <Text className="text-textSecondary text-sm">{matchedContact ? callState.number : "Mobile"}</Text>
                    )}
                </Animated.View>

                {!showKeypad ? (
                    <Animated.View entering={ZoomIn.delay(300).duration(400)} className="items-center my-5 flex-1 justify-center">
                        <View className="w-[120px] h-[120px] rounded-[60px] bg-primaryLight justify-center items-center">
                            <FontAwesome name="user" size={56} color={theme.colors.primary} />
                        </View>
                    </Animated.View>
                ) : (
                        ...
                        {DTMF_KEYS.map((row, rowIdx) => (
                            <View key={rowIdx} className="flex-row justify-evenly mb-3">
                                {row.map((digit) => (
                                    ...
                                    >
                                        <Text className="text-textPrimary text-[28px] font-light">{digit}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        ))}
                        ...
                        <View className="flex-row justify-between">
                            <View className="items-center">
                                <TouchableOpacity
                                    ...
                                    style={{ shadowColor: theme.colors.danger, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 }}
                                    onPress={handleReject}
                                >
                                    <FontAwesome name="phone" size={32} color="white" style={{ transform: [{ rotate: "135deg" }] }} />
                                </TouchableOpacity>
                                <Text className="text-textSecondary text-[13px] mt-2 font-medium">{"Decline"}</Text>
                            </View>
                            <View className="items-center">
                                <TouchableOpacity
                                    className="w-[60px] h-[60px] rounded-[30px] bg-primaryLight justify-center items-center"
                                    onPress={() => {
                                        try { CallLogsModule.silenceRingtone?.(); } catch (_e) { }
                                    }}
                                >
                                    <FontAwesome name="bell-slash" size={24} color={theme.colors.primary} />
                                </TouchableOpacity>
                                <Text className="text-textSecondary text-[13px] mt-2 font-medium">{"Silence"}</Text>
                            </View>
                            <View className="items-center">
                                <TouchableOpacity
                                    className="w-[75px] h-[75px] rounded-[37.5px] bg-success justify-center items-center"
                                    style={{ shadowColor: theme.colors.success, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 }}
                                    onPress={handleAnswer}
                                >
                                    <FontAwesome name="phone" size={32} color="white" />
                                </TouchableOpacity>
                                <Text className="text-textSecondary text-[13px] mt-2 font-medium">{"Accept"}</Text>
                            </View>
                        </View>
                    ) : (
...
const ActionButton = ({
    ...
    >
        <FontAwesome name={icon as any} size={24} color={active ? theme.colors.white : theme.colors.primary} />
        <Text
