...
export default function ContactDetail() {
    ...
    const handleSave = useCallback(async () => {
        ...
        setSaving(true);
        try {
            const updatedContact: Partial<Contacts.Contact> = {
                id: id,
            ...
            await Contacts.updateContactAsync(updatedContact as any);
            refresh();
            setEditing(false);
            // Reload contact
            const result = await Contacts.getContactByIdAsync(id, [
    ...
    if (!contact) {
        return (
            <View className="flex-1 bg-background justify-center items-center pt-[StatusBar.currentHeight || 0]">
                <Text className="text-textSecondary text-[17px]">{"Contact not found"}</Text>
            </View>
        );
    }
    ...
    return (
        <View className="flex-1 bg-background pt-[StatusBar.currentHeight || 0]">
            <View className="flex-row items-center justify-between px-4 py-3.5 border-b-[0.5px] border-border">
                <TouchableOpacity onPress={() => router.back()} className="p-1 min-w-[60px]">
                    <FontAwesome name="arrow-left" size={20} color={theme.colors.textPrimary} />
                </TouchableOpacity>
                <Text className="text-lg font-semibold text-textPrimary">{editing ? "Edit Contact" : "Contact"}</Text>
                {editing ? (
                    <TouchableOpacity onPress={handleSave} disabled={saving} className="p-1 min-w-[60px]">
                        <Text className="text-base font-semibold text-primary text-right" style={[saving && { opacity: 0.5 }]}>{"Save"}</Text>
                    </TouchableOpacity>
                ) : (
                    <TouchableOpacity onPress={() => setEditing(true)} className="p-1 min-w-[60px]">
                        <Text className="text-base font-medium text-primary text-right">{"Edit"}</Text>
                    </TouchableOpacity>
                )}
            </View>
            ...
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} className="flex-1">
                <ScrollView className="flex-1 px-5" keyboardShouldPersistTaps="handled">
                    {/* Avatar and name */}
                    <Animated.View entering={FadeInDown.delay(100).duration(400)} className="items-center py-6">
                        <Animated.View entering={ZoomIn.delay(200).duration(400)} className="w-[90px] h-[90px] rounded-[45px] bg-primaryLight justify-center items-center">
                    ...
                    {!editing ? (
                        <Animated.View entering={FadeInDown.delay(200).duration(400)} className="flex-row justify-center gap-6 mb-6">
                            <TouchableOpacity className="w-[80px] h-[70px] rounded-2xl bg-primaryLight justify-center items-center gap-[6px]" onPress={() => handleCall(phones[0]?.number)}>
                                <FontAwesome name="phone" size={20} color={theme.colors.primary} />
                                <Text className="text-xs text-textSecondary font-medium">{"Call"}</Text>
                            </TouchableOpacity>
                        </Animated.View>
                    ) : null}
                    ...
                    <Animated.View entering={FadeInDown.delay(300).duration(400)} className="mb-6">
                        <View className="flex-row justify-between items-center">
                            <Text className="text-sm font-semibold text-textSecondary mb-2.5 uppercase tracking-[0.5px]">{"Phone"}</Text>
                            {editing ? (
                        ...
                        {editing
                            ? phones.map((phone, index) => (
                                <View key={index} className="flex-row items-center">
                                    ...
                                    />
                                    {phones.length > 1 ? (
                                        <TouchableOpacity onPress={() => removePhone(index)} className="p-2">
                                            <FontAwesome name="minus-circle" size={20} color={theme.colors.danger} />
                                        </TouchableOpacity>
                                    ) : null}
                                </View>
                            ))
                            : phones.map((phone, index) => (
                                ...
                                >
                                    <View>
                                        <Text className="text-xs text-textSecondary capitalize mb-0.5">{phone.label}</Text>
                                        <Text className="text-base text-textPrimary">{phone.number}</Text>
                                    </View>
                    ...
                    {editing ? (
                        <View className="mb-6">
                            <Text className="text-sm font-semibold text-textSecondary mb-2.5 uppercase tracking-[0.5px]">{"Email"}</Text>
                            <TextInput
                                className="border border-border rounded-xl px-4 py-[14px] text-base text-textPrimary mb-2.5 bg-card"
                    ...
                    <Animated.View entering={FadeInDown.delay(400).duration(400)}>
                        ...
                        >
                            ...
                            <Text className="text-base font-medium text-danger">{"Delete Contact"}</Text>
                        </TouchableOpacity>
                    </Animated.View>
